"""
descargar_rubro08_chiclayo.py
-------------------------------
Descarga los 5 archivos .xls de Consulta Amigable (MEF) que alimentan
historico-rubro08.html para la Municipalidad Provincial de Chiclayo (MPC).
Mismo motor robusto ya validado en mpl-ingresos/scripts/descargar_rubro08.py
(sesión 20-ago-2026): clic_con_reintento() con polling 18s / 3 intentos
con escalada a doble clic, checkpoint de fila (radio input:checked) y
pausa de 2s entre archivos.
...
Conceptos cubiertos (6, definidos por el jefe) -- MPC no tiene el
concepto "Sanciones Tributarias" que sí tiene MPL, por eso son 5
archivos en vez de 6:
  predial.xls           -> Predial Corriente + Regularización Predial (2 conceptos)
  alcabala.xls           -> Alcabala
  patrivehicular.xls     -> Al Patrimonio Vehicular
  impuestoselectivo.xls  -> Impuesto Selectivo a Servicios Específicos
  intereses.xls          -> Intereses

MODO DE USO (desde la raíz del repo orad-ingresos-chiclayo):
    python scripts/descargar_rubro08_chiclayo.py
"""
...
# Pasos comunes a los 5 archivos: TOTAL -> ... -> Municipalidad (MPC).
PASOS_MPC = [
...
# Rutas confirmadas con codegenrubro08.txt (24-ago-2026) y cruzadas
...
# Motor validado en descargar_rubro08.py / descargar_rubro09.py (MPL)
# --------------------------------------------------------------------
...
def git_push_automatico():
    ...
    print("     Siguiente paso: python scripts/actualizar_json_rubro08_chiclayo.py")
        ...
        for i, config in enumerate(ARCHIVOS, 1):
            ...
            try:
                ...
                try:
                    ...
                    pass
                # Un archivo fallido no detiene el lote -- sigue con el siguiente.
